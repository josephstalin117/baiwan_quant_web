<template>
  <div class="container">
      ...
      <b-form-select v-model="accNumber" :options="accOptions" :select-size="4">
      </b-form-select>
      ...
      <b-button type="submit" variant="primary" @click="saveNote">提交</b-button>
      <b-button type="reset" variant="danger" @click="initNote">重置</b-button>
    <!-- </b-form> -->
   ...
   data() {
        ...
        companies: {
            handler: function(val){
              ...
              for(let i = 0; i < val.length; i++){
                ...
                // val[i].value = (val[i].value).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
                // val[i].last_value = (val[i].last_value).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
    ...
    methods: {
      ...
      saveNote: async function () {
        const url = this.$common.baseUrl + "acc_noter/savenote";
        this.filteCompanies();
        let response = await this.$http.get(url, {
          params: {
